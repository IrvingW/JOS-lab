...
void (*fun_ptr_g)(void);
struct Gatedesc volatile *entry;
struct Gatedesc old;
...
void ring0_call(void (*fun_ptr)(void)) {
    ...
    //        file if necessary.
        ...
        struct Segdesc *gdt = (struct Segdesc *)(ROUNDDOWN(gdt_page + PGSIZE, PGSIZE) + PGOFF(gdt_pd.pd_base));
        entry = (struct Gatedesc *)gdt + (GD_TSS0 >> 3);
        old = *entry;
        fun_ptr_g = fun_ptr;
        SETCALLGATE(*entry, GD_KT, call_fun_ptr, 3);
